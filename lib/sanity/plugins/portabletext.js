...
const Code = ({ value }) => {
  ...
  // Graceful fallback if the language isn't registered at runtime
  if (typeof Refractor.hasLanguage === "function" && !Refractor.hasLanguage(language)) {
  ...
  try {
    ...
    const fallback = rawLanguage === "tsx" ? "typescript" : "javascript";
    return (
      <Refractor
        language={fallback}
        value={value.code}
