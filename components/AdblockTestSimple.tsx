...
import useAdblockDetection from '@/hooks/useAdblockDetectionFixed';

// Simple test component to verify adblock detection works
export default function AdblockTestSimple() {
  const [showTest, setShowTest] = useState(false);
  const { isAdblockDetected, isChecking, detectionMethod, retryDetection, isBlocked } = useAdblockDetection();
    ...
    return (
      <div className="fixed bottom-4 right-4 z-50">
        <button
          ...
          className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700 shadow-lg"
        >
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md w-full">
        ...
        <div className="space-y-3">
          ...
          <div>
            <span className="font-medium text-gray-700 dark:text-gray-300">Blocked: </span>
            <span className={`px-2 py-1 rounded text-sm ${
              isBlocked 
                ? 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200'
                : 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200'
